...
// tracking vars
const started ={};// current running plant
let disconTime=[], disconPlant=[];// track plant disconnected x recovery it after a socket reconnection 
...
const DEBUG_savings=true; // dont increment savings array
const PRTLEV=8;// print log level, >5 many prints! 
//const PRTLEV=5;// print log level, <=5 less prints! 
...
app.post("/registerPlant/", (req, res) => {// 22122023,see the  app.post("/registerPlant_/",
                                            // this post (from form) is called when ha register for a plant configuration passing json cfgdata or a sintesis of it
                                            // curl -H "Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJiNmYxMDk3NDYxODI0YmZhYjkwNTc2NjQ1ZDVmODU4MyIsImlhdCI6MTY5NDQ0Mjg2MiwiZXhwIjoyMDA5ODAyODYyfQ.ppeuf-Ma1vLVQCT0Qrt07C5TXGHsHasX3ElOl1NCX3A"
                                            //  -H "Content-Type: application/json" 
                                            //  -d '{"filename": "yaml_ex.txt","yaml":"atextyaml",cfgdata:"{\"casinauser1_API\": {\"cfg\": {\"Ent_Prefix\": \"casinauser1_\",\"name\": \"casinauser1_API\"}}}}' 192.168.1.214:8080/registerPlant
...
let cfgdata,userEnt,
reset=false;//if plant exist use it 
if(req.body.cfgdata){
    // to try 
    cfgdata=JSON.parse(req.body.cfgdata);// get data from user form
 
console.log('post /registerPlant/  read cfgdata: ',cfgdata);
cfgdata.result='ok';// set validation flag ok 
...
userEnt={ // these dev are ha real local mqtt switch . its name diffears from the cfg template whose name are like casina implementation with a prefix : Ent_Prefix
          // scaldabagno is updated by an action on a automation on event fired by portid=55 ?  dev (preferred gas/pdc) by dev custF,  see setSwitch()
           ...
           //  rssi is updated directly by service fired by  update entity using updating dev msg  msg.payload , see setSwitch()
          // nb custF will fire an event based on updating dev msg and last state
            consCaldaia:'switch.rssi',consAcs:'switch.acssw',term:'sensor.shelly_ht_temp',humid:'sensor.shelly_ht_humid',
            consCaldaia_:'rssi',      consAcs_:'acssw',       term_:'shelly_ht_temp',       humid_:'shelly_ht_humid',
          scaldabagno:'switch.scaldabagno',scaldabagno_:'scaldabagno'};
// todo: to avoid 2 entry for a switch:  use a function to delete char before '.' and '.' itself !
// nb if different user works on same entities all plant works on same entity !, use only for debug !

// insert here the debug conf :
cfgdata={Ent_Prefix:plantUser+'_',//=plant+'_';//   the prefix to put in the plant entity name to generate a ha plant clone for a ner user plant  HHYY
                                  // sostituisce il £ nei entity name del template , see  haCfgData_() , defFVMng() ! 
        connCfg:{  // def cfg ..... LLHH resolve with clientobj=clientObject(client)
        ...
        userEnt,
        user:plantUser,email:'luigi.marson@gmail.com',token:'123',pass:'pass',invNomPow:5,huawei:{inv:"1000000036026833",bat:"1000000036026834",credential:null}}; // init for debug, just to store, during user/plant registration, the user specific data localEntity to use in addUserPlant() 
}
let plantN=plantUser+'_API';


if(cfgdata)reset=true;// rewrite config 
...
// if(Gpio) LED= new Gpio(5, 'out'); //use GPIO pin 4 as output , all plants can use this !!!!!
...
const getio=require('./nat/io/getio.js').init(Gpio,rs485,pdate,PRTLEV);// *****************************
...
ccbbRef=function ccbb(plantname) {
  ...
  if (plantname) {
    let name = plantname;
    console.log('ccbb  name:',plantname,' instance alredy started? : ',started[name]!=null);
...
function program(state, inp__, probes) {// /  probes={giorno:19.2,notte:,,,,}   // 082024 todo l'individuazione dei devices dovra essere fatta  usando configured funz like consolidate() !
  ...
  /*
  inp__=sched:  {
          inp_=programs: { giorno: { sched: [{"8:30": 15,"17:00": 19,}],    set in HHDD  
  ...
  */
  ...
  let toactivate ,// [actiongiorno,actionnotte, actionscant,actionacs]
  ...
  if (probes && inp) {// inp=sched={giorno:{'16:10':-3,,,,},notte:{},probMapping:[],mapping:[],ei} 
    ...
    */
    ...
    console.log('program() calculate activation for termostat of  giorno , notte, scant, pdc : ',toactivate);

    // TODO TODO only 2 zones giorno notte + acs items  of toactivate can be !=[false,false] (if have programs and probe(but not acs))
    //  cioe scantinato is ever [false,false]

    state.lastProgramAlgo = state.lastProgramAlgo || { model: 'programbase' };

    let changing = false;
    if (activation) {// some section (now giorno notte only ) is cooler than programmed in winter or warmer in summer
      ...
      */
      ...
      console.log('programming() runned x plant ',plant,' at ',date.toLocaleString(),' :found some unsatisfacted programmed temp in house, so suggests virtual  (like [heat,pdc,g,n,s,split]) relays updates: ', ret,'\n and  anticipate relays updates:',antret);
    ...
    } else {//  no one of (giorno , notte )programs has temperature out of  target on current time slot 
    ...
    state.lastProgramAlgo.probes = probes;// current T for zones 
    state.lastProgramAlgo.program = inp;
    state.lastProgramAlgo.pumps = ret;// action if not anticipating 
    state.lastProgramAlgo.anticGap = antret;// action if  anticipating 
    state.lastProgramAlgo.desT = desTemp;// current desidered t and desidered t on anticipating,extracted from programming schedule + tollerance x anticipating
                                          // temporarely giorno , notte   ( add acs ?)

  } else {
    state.lastProgramAlgo = false; optimRet = null;
  ...
  */
  ...
  function toact(zona, sonda, sc_, isSummer, desT = [21,21]) {// zona,valore sonda, sc=programma orario x la zona. verifica se una zone ha temperatora sonda <  alla programmata nello slot dove cade l'ora corrente
    // se  torna true
    // nb returns/updata desT[0] too!
    /* sc_= 				      {sched: {"8:30": 27,"17:00": 30,},
                            toll: {"8:30": 0,"17:00": 0,},// same orari che in sched!, temporaneamente ricavato da TTollerance : 
                            TTollerance: 0,
                            },    */
    ...
    let sc = sc_.sched, scantic = sc_.toll;//  scantic = [{"8:30": 0.5,"17:00": 0,6}]
    let DELTA; if (!scantic) DELTA = sc_.TTollerance || 0; else DELTA = null;// use if .toll is null or  0
    ...
    if (PRTLEV > 4) console.log(' toact() analizing  zona: ', zona, ' programma : ', sc, ',  at present hour:min  ', h, ':', m,);
    ...
    for (let i = 0; i < keylist.length; i++) {//
      ...
      if (PRTLEV > 5) console.log(' toact() analizing  zona: ', zona, ' orario: ', keylist[i], ',  with present hour min : ', h, m,);

      if (h < ma[0]) {// 10 < '8'
        // got! ends scan the index in sc is slot
        slot = i - 1; i = 1000;// got slot
        //tempx =tempx + ' - got slot '+ slot ;// ma[0]; 
        if (PRTLEV > 6) console.log(' got slot ', slot, ', because present hour is less then slot+1 hour');
      }
        ...
        if (!(h > ma[0]) && m < ma[1]) {
          ...
          if (PRTLEV > 6) console.log(' got slot  ', slot, ' because present hour is = but current min is < !');
        }
      }
    ...
    desT[0] = temp;// trace back//updates the desidered temp if no anticipating 
    ...
    if (PRTLEV > 5) console.log(' toact() activate generator: ', act, ' . infact found current time slot: ', slot, ' with desidered temp: ', temp, ' and probe temp ', sonda, ' . modalità condizionatore/riscaldamento: issummer: ', isSummer);
    if (PRTLEV > 5) console.log(' toactantic() activate generator: ', actantic, '  . infact found current time slot: ', slot, ' with desidered antic temp: ', tempantic, ' and probe temp ', sonda, ' . modalità condizionatore/riscaldamento: issummer: ', isSummer,', delta: ',delta,', updating desT: ',desT);
      ...
      repeatcheckxSun: function (hourin, hourout, period, execParm, cb2) {// register the procedure setinterval to repeat, period in minutes
        ...
        if (goon);//;// one day start immediately
          ...
          interv = setInterval(function () { // Set interval for checking, never stop till the repetion ends 
            ...
            console.log(' repeatcheckxSun : setinterval  handler (start repetition job) called x procName: ', execParm.procName, ',  at hour: ', dh, '(dOraLegale:',dOraLegale,') and minutes: ', dm, '. present time norm: ', dateN, 'must be in: ', minIn, '-', minOut);
...
function setPump(pumpnumber,on,fn){// 0,1,2,3    on : changing value (true/1 or false/0)
  ...
  let relaisEv=state.app.plantconfig.relaisEv,
         ...
         getCliDiscon() banalmente ritorna questo onconnection handler clientDiscon (significa il browser non connesso, session expired !)!! 
          in base a clientDiscon si  settera getcontext.discFromBrow true se clientDiscon=false cioe non processo il doppio comando in onRelais
           nb getcontext.processBrow() ritorna !getcontext.discFromBrow , e' sinonimo!
           inoltre  when clientdiscon it useless run function that will emit browser .emit , because no connection is available
        */
  clientDiscon=fn.getcontext.getCliDiscon();// attenzione duplica  onconnection handler clientDiscon con stesso nome


    // set info flags about socket connection state  in .state
  state.clientDiscon=clientDiscon;
  console.log(' setpump() will emit socket pump event (+ direct onrelays()) plant: ',state.app.plantname,', to change browser relay value x pump number',pumpnumber,' asking to set: ',on,', current browser socket is available: ',!clientDiscon,' with session: ',fn.getcontext.getSession());
  ...
  console.log(' .....  so blocking flag (double browserprocess) for onRelais()  is ',fn.getcontext.discFromBrow);// =clientDiscon=true
...
if(PRTLEV>4)console.log(' watchparam ( server button handler, called also by algo activated setPump() ) : it is firing a socket event pump for pump: ',pumpName,' so in browser well set pump flag : ', value,' now  try to send ws (ws.id=',socket.id,') pump emit '); //output error message to console
lightvalue = value;
if(socket){socket.emit('pump',pumpName, lightvalue); //send button status to browser client, if available  
          // >> pumpName in relaisEv
           ...
           if(PRTLEV>7)console.log(' watchparam, pump emit was sent on wsid: ',socket.id);}
...
function setManual (pump_, val, coming,checked,eM,hour)  {// this will be like the final part of a manual algo: propose a single pump value to be evaluater with all other active algo by optimize
  ...
  if (eM) {
    ...
    eM.state.lastUserAlgo.expAfterHour = hour;
...
io.sockets.on('connection', function (socket) {// WebSocket Connection :server is listening a client browser so now we built the socket connection, transmit to server if there are status updates 
                                              // mainspace
                                              // this is the .on connection handler . it is a closure of function that use closure state , like eM
                                              //  eM e' settato da socket.on('startuserplant',...    see eM setting !
...
let session = socket.request.session;// session used in other tcp request ??, if only socket are used the user is avaible here for all socket connection closure !
...
console.log('on connection got from a browser set in login, user: ',user,`new  session: ${session.id}  , socket connection id ${socket.id}`,
            ...
            '\n  ... eM is null : ',eM==null,', so in case of socket reconnect the browser (has a plant view active) can ask plant event that require the recovery of eM ! ',
            ' \n  ... so do eM recovery!');
console.error('on connection got from a browser set in login, user: ',user,`new  session: ${session.id}  , socket connection id ${socket.id}`,
            '\n   ... so clientDiscon is set false ',
            '\n  ... eM is null : ',eM==null,', so in case of socket reconnect the browser (has a plant view active) can ask plant event that require the recovery of eM ! ',
            ' \n  ... so do eM recovery!');

// see if this is a socket reconnection
let ctime=pdate().getTime(); 
if(disconTime.length >0 &&ctime-disconTime[disconTime.length -1]> 5000){// too old so its a new connection , continue normally
disconPlant=[];disconTime=[];// reset 
}else{// recover session and plant name ,recover eM

 disconPlant.forEach((dt,ind)=>{// 1 discover the plnt disconnected and ask user what want to reconnect 
                            // or 2 if the session remain the old reconnect to plat with same session 
  // if(dt.plant)
  // 2:
  console.error('on connection discovered recent socket reconnection for session: ',disconPlant[ind].session,' plant: ',disconPlant.plant,' so recover it !!!!! ');
  if(ctime-disconTime[ind]<5000&&disconPlant[ind].session==session.id&&started[disconPlant.plant]&&started[disconPlant.plant].instance){
    eM=started[disconPlant.plant].inst;
    eM.getcontext.socket=socket.id;// was set at discon
    eM.getcontext.clientDiscon=false;

    disconTime[ind]=0;// exclude for future
  }
...
console.log(`saving user and socketid ${socket.id} in session ${session.id}.it will be  used by future fn ( when  associated to user plant). in case algo procedure wants to do something with session and socket if they are active`);
...
socket.on('createUsrPlant',// to do emit in browser.  create the plant config in models , with yaml file in plant.yaml={dashboard,package}
  ...
  function(cfgdata,plantUser,browserUser){

    addPlantHandler(cfgdata,plantUser,plantUser+'_API',
  ...
  socket.on('startuserplant', function (plant_, feat) { // user press button to connect to some plant, so this event is fired , feat url enc
    ...
    // must be nullified (cioe tutte le funzioni del closure non devono essere referenziate all'esterno del closure , ex closure return a closure func) when socket disconnect so closure can be garbagecollected 
    if (!eM.reBuildFromState) {let prevSes;if(eM.getcontext.getSession)prevSess=eM.getcontext.getSession().id;// eM instance is recovered in plants , getcontext represent the socket .on(connect ) closure state , ex session,socket
      console.log(' startuserplant , user ', user, ', plant: ', plant_,'  at: ',pdate(),'  recovered a plant ctl(eM/fn) already running ,changing, eM.getcontext, ctl session/socket browser context data. old/new session id are: ', prevSess, '/', session.id,' new socketid: ',socket.id);
    

    } else console.log(' startuserplant , user ', user, ', plant: ', plant_, '  at: ',pdate(),', no running plant ctl found . setting session/socket context data. no em.getcontext, so new session id are: ', session.id,' new socketid: ',socket.id);
    ...
    if (!eM.reBuildFromState&&eM.iodev) {// only not eM.reBuildFromState (recovered from runtime plants) has already the devices !)
...
if(!(mqttInst=mqtt.init(plantconfig,PRTLEV))){// 02052023: now work with instance not the class , must be passed to getio.js , and  something else ? ....
...
if(PRTLEV>2)console.log(' abilita2(),got dev ctls x models.js lists: gpionumb,mqttnumb. relais map is: ',devices.devmap);
if(PRTLEV>2)console.log(' abilita2(),got dev ctls x models.js lists:,mqttprob. probs map is: ',probes.devmap);// devices.devmap);
...
builddev(devices,eM.iodev.relais_,state.devMap,0);  // reset pumpsHandler[ind], to call browser socket  emitters x pumps
                                                    ...
                                                    //                                                                       and the ctl of dummy dev with portid=0 and index >=relaisEv.length ( usually  index >=relaisEv.length ()
...
// to do check the relais are not alredy set to other plant/eM, in this case pushing button will set the pump for all plant that has a socket connected to the browser 
...
/*
if(relais&&relais[ind])relais[ind].watch(pumpsHandler[ind]);// attach same handler watchparam(pump) to all gpio browser pump  buttons (gpio input same behaviour then lick on browser button )
...
if(eM.iodev.relais_[ind]&&eM.iodev.relais_[ind].cl&&(eM.iodev.relais_[ind].cl==2||eM.iodev.relais_[ind].cl==1)){// the dev is a mqtt var , see VVCC in howto
 ...
 // if(!isNaN(val_=data.payload)&&!isNaN(checked_=data.checked)){// payload is integer, so checked : if is not 0/1 or '0'/'1'
   if (Number.isInteger(val_=Number(data.payload))&&Number.isInteger(checked_=Number(data.checked))&&
   !Number.isNaN(hour=Number(data.data))){
   
    console.log(' abilita2 interrupt Iter1 : receiving   var ',pump,', msg val (0/1) : ',val_,'  checked : ',checked_,' hour: ',hour,' , actual status is :',state.relays[pump],' , queue is :',queue,', data was ',data);
  ...
  setManual (pump, val_,"ext cmd",checked_,eM,hour);// val_ /checked_ =0/1
...
console.log(' abilita2() could not find intwebsoc device in iodev.relais_ list')
}
...
function intWebSock(val=0, devqueue, message){// val=0/1, the int handler of a formatted request serving url mqttxwebsock
  ...
  //message is json obj got from  : message={"payload":1,"sender":{"plant":"Casina_API","user":77},"url":"mqttxwebsock","event":"repeatcheckxSun","data1":[1,23,2,{"FVPower":30}]} 
  ...
  if(mevent=="stopcheckxSun"){
    ...
    stopRepeat();
      ...
      startAnt=true;
        ...
        {
          let triggers2={};triggers2=message.data[3];// only "PGMgiorno" : { "8:30" : 27, "17:00" : 30 },"TgiornoTollerance":1.4
...
/*                                                                    full triggers2={
                                                                          ...
                                                                          probMapping: "==&&state.probmapping=[0,1,2]",
                                                                          ei: "W",
...
if(triggers2&&triggers2.PGMgiorno){ // complete with std data
  triggers2.mapping="==&&state.devmapping=[0,1,2,3,-1,5]";
  triggers2.probMapping="==&&state.probmapping=[2,4,0,3,1,5]";

  if(triggers2.ei==null)triggers2.ei=message.param.ei||"W";// >>>>>>>>>>>>>>>>>>>>><  old way 

  triggers2.Tgiorno=true; 
...
*/
  ...
  socket.on('light', function (data) { //handler of  'light' event fired on browser ,get light switch status from client web page .   data=0/1 ?
    ...
    if(LED)LED.readSync().then((led)=> checkit(led));

    function checkit(val){
    if (lightvalue !=val) { //only change LED if status has changed
      if(LED)LED.writeSync(lightvalue); //turn LED on or off
      /*if (buttoncaused) {
        buttoncaused = false;
      } else {
        // eM.emit('web', 1);
...
// moved 
 function onRelais_  (pump,data) { // dead code //pumps unique handlerget pumps switch status from client web page.  data =0/1
  ...
  if (relais_[pump]&&lightvalue != relais_[pump].readSync()) { //gpio comanding relays is called, only change LED if status has changed
    if(relais_[pump])relais_[pump].writeSync_(lightvalue,{}); //turn LED on or off
    /*
...
function setanticipateflag(set_,algo,activeAlgoRes=null){ //  AAQQOO  
                                                    ...
                                                    // - otherwhise reset algo init param  e algo result (state[activeAlgoRes]=null) (so stopping ? )
  ...
  if(!eM){console.log('.. setanticipateflag() eM is null ');
  console.error('.. setanticipateflag() eM is null ');
}else console.log(' . setanticipateflag(), eM is found '); 
    console.log(' setanticipateflag() called to set running algo: ',algo,' init param: ',set_,' , in state.',algo,' ,(if null init parm will also  reset state.',activeAlgoRes);
    //if(set_)
    anticipateFlag(set_,eM,algo,activeAlgoRes,session);// eM is set before in a preceeding  socket.on('startuserplant',,, ( like create a  closure var)
}
...
function repeatHandler1(starthour,stophour,dminutes,triggers2,custFs_param={scaldabagno:
                                                              {event:'scaldabagno',attName:'hours',attVal:3,event0:'resetScaldaB'}// {entity:'switch.scaldab',hours:3}
                                                              }) {// triggers2 keys sono gli items di :extract2 =  ["Tgiorno","PGMgiorno","Tnotte","PGMnotte","lastT","mapping","probMapping","ei","TgiornoTollerance",,,,],
                                                                // 27012024 : params are of 2 types:
          ...
          if(eM){ eM.state.customParam=eM.state.customParam||{};
                  eM.state.customParam.scaldabagno=custFs_param.scaldabagno;// add in cust param a entry x scaldabagno , used by some dev as conig in custF(). or function of . GGSS
                                                                            // can also merge all custFs_param properties !
                                                                            // todo add in start program form the name of scaldabagno and the number of hour to power it 
                                                                            //  in dev gas/pdf we add a custF that start ha event :
                                                                            //    - ha event start a timer x 3 hour but checking that battery is more then pmax/4
                }
                                                                /*  
                                                                          Tgiorno: true,
                                                                          PGMgiorno: {
...
if (TToll) {// non più ! o si ? per sicurezza prevediamo siano valori numerici o stringhe e convertiamo tutto in float 
prog.toll = {};// // tolleranze (un valore x all), same keys of sched.giorno.sched
for(el in TToll){
  let mv=TToll[el];
  if (mv !='' && (!Number.isNaN(val_=Number(mv)) ))   prog.toll[el] =val_;// ok
  else   prog.toll[el] = 0;
...
for (let ii = 0; ii < ks.length; ii++) {
prog.toll[ks[ii]] = prog.TTollerance ;
}
}
}
}
...
function stopprogrammer() {
  ...
  socket.on('disconnect',() => {// to do 
    ...
    console.error(' .on(disconnect : socket connection lost');
   if(eM&&eM.getcontext){ console.error(' .......conn lost  at: ',pdate(),' on  plant: ',eM.state.app.plantname,', session ',eM.getcontext.getSession().id,' socketId: ',eM.getcontext.socket.id,
                                     '\n  socket.on disconnect,  so clientDiscon is set false ');
                                     session.id=null;//or session=null;
                                     eM.getcontext.socket=null;
                                     eM.getcontext.clientDiscon=clientDiscon=true;
                                     disconTime.push(pdate().getTime()); disconPlant.push({plant:eM.state.app.plantname,session:eM.getcontext.getSession().id});
  }else console.error(' .......conn lost  at: ',pdate(),'  eM is null!');
    clientDiscon=true;
...
process.on('SIGINT', function () { //on ctrl+c
  ...
  if(LED)LED.writeSync(valCorrection(0)); // Turn LED off
  if(LED)LED.unexport(); // Unexport LED GPIO to free resources
...
function anticipateFlag(set_,fn,algo,activeAlgoRes,session=null){// like onRelais, write state after completed it to store anticipate algo init param// algo =anticipate/program/manual
  ...
  if(!fn){console.error('anticipateFlag(), eM is null!, probably after a debugging stop point or a restart session without a plant set. cant goon, session: ',session);
 }else {
  console.log(' anticipateFlag(), eM is found ');
  let state=fn.state;
...
async function onRelais(pump,data,coming,fn) { //pumps unique handlerget pumps switch status from client web page  data=0/1 pump ='pdc',,,
  ...
  let session=null,
   clientDiscon=true;// attenzione variabile locale di onRelais, stesso nome della locale in socket closure handler !!! 
   if(fn.getcontext){session=fn.getcontext.getSession();clientDiscon=fn.getcontext.getCliDiscon();}// rettifica immediata !
  ...
  const useCb=true;// works only with true
  if(pump_>=0){// found
    ...
    if (coming && coming == Serv_) {// set a expiring flag if coming from server. when comes browser duplicated command il the flag is set and valid we dont care 
      if(PRTLEV>3)console.log('OnRelais started x mqttnumb dev , pump/devName: ', pump, ', plantname ', fn.state.app.plantname, ' , session id ', sessionid, ', socket id ', socketid, ', to set ', data, ',  coming from ', coming, 
      ' the socket connection with browser is active : ', !clientDiscon ,' and, if true , set process browser request: ',fn.getcontext.processBrow());
      ...
      if (!useCb) {// old , from server
        if (!clientDiscon)// if client is disconnected we will anyway blocking double browser req , so it is useless 
          ...
          if (fn.state.discFromBrow && state.blocking) {
            clearTimeout(state.blocking);// fn.state.discFromBrow : the server req  (onRelais ) is followed by a browswer req : 
            //    so if a previous browser timeout was set , reset it and  reset the interval of browser  blocking 
            state.blocking = setTimeout(() => { fn.state.discFromBrow = false; state.blocking = null }, 2000);// discard browser req, server req  is enougth
          }
      }else ;// new coming from server : do nothing
      ...
      if (!useCb) {// old
        if (//             // if the socket conn is became disconnected , this req cant come here , so the connection is active, so check if timeout is
          ...
          if(PRTLEV>3)console.log('OnRelais started x mqttnumb dev , pump/devName: ', pump, ', plantname ', fn.state.app.plantname, ' , session id ', sessionid, ', socket id ', socketid, ', to set ', data, ',  coming from ', coming, ' but it comes on a not expired time out on which the duplicated req is expected  ');
      ...
      } else {// new code :  coming from browser
              // the server already sent this request so ask to not run this browser duplicated request. better let browser not to send duplicated request received from browser !
        if (fn.getcontext && fn.getcontext.processBrow){
          if(PRTLEV>3)console.log('OnRelais started x mqttnumb dev , pump/devName: ', pump, ', plantname ', fn.state.app.plantname, ' , session id ', sessionid, ', socket id ', socketid, ', to set ', data, ',  coming from ', coming,
          ' the socket connection with browser is active : ', !clientDiscon , ', so the server asks dont process this browser req (because is duplicated) : ', fn.getcontext.processBrow(),
           '\n  so reset disconFromBrow=false and return');
   
          if (!fn.getcontext.processBrow()) {
                    // new: meglio resettareil blocco settato in setPump():

            if(fn.getcontext.blocking)clearTimeout(fn.getcontext.blocking);// fn.state.discFromBrow : the server req  (onRelais ) is followed by a browswer req : 

            fn.getcontext.discFromBrow=false;// so in eM.getcontext.processBrow() return true if fn.state.discFromBrow=false
            
            return;// the server already sent this request so ask to not run this browser duplicated request. better let browser not to send duplicated request received from browser !
          }
      }}
    }
    ...
    if (ctl) {
      ...
      console.log('OnRelais  , coming from ', coming,' accept processing pump/devName: ', pump, ', found with (only if mqtt) index ', ctl.devNumb, ', devid/portid ', ctl.gpio, ', is a mqttdev?: ', !!ctl.cfg);
      ...
      curval = null;// 0;
      if(PRTLEV>7)console.log(' onRelais, coming from: ',coming,', current rele position x dev name ',pump,', is ',curval,' asking to set : ',data); 
 ...
 // now ever write  if (!curval||data != curval) 
  ...
  { // 0/1 != 0/1 gpio comanding relays is called,>>>>> only change gpio if current position/value is different from present hw relay value !!
    if(PRTLEV>4)console.log(' onRelais,  changing/confirm current rele hw position/value x ',pump,' , index ',pump_,' ,  to new value (nb inverted val 0 <> 1): ',data); 
    /*
   let custParm=state;// Nb state.customParam sono i pram x il custF ! . 27012024 can be [paramxdevN0={},,,,,,] load when start program . ex set param x slit custom dev or attribute for ext_entity to set defined in cfg customDevParam (like state) 
   ...
   if(pump_=='split'){// insert cust dev param only x split  dev 
    ...
    if(fn.iodev.relais_[pump_])fn.iodev.relais_[pump_].writeSync_(valCorrection(data),state); // state={customParam, ,,,}, customParam sono i pram x il custF   XXTT
    if(PRTLEV>7)console.log(' onRelais,  todo : verifying current rele  position/value changing  x ',pump,' now is: ',data); 
    ...
    // 
    if(PRTLEV>7)console.log(' onRelais(), to update, now call writeScriptsToFile: ',plantname,' scripts/state: ',state);
...
// end news::
  ...
  state.relays_Attr=  state.relays_Attr||[];// just x sure


    // set attributes x split custF,  depending on zones to activate ?
    ...
    let split=state.app.plantcfg.relaisEv[5];// the name of 5+1 th relay is split
    state.relays_Attr[5]={};//{split}// copy present directiveparam x user of relays. ex : custF
    if(state.relays[split]){// directive wants to start split
      if(res[2]||res[3])// activate all splits if giorno o notte 
      state.relays_Attr[5].custDev={custSet:0,// 0 <> 1 , invertet values !, 0 means turn on .  custSet=inverted val !
        quiet:false,
        section:false};// means all split
    }else{
      state.relays_Attr[5].custDev={custSet:1,// 0 <> 1 , invertet values !, 0 means turn on 
      quiet:false,section:false};
    }


  console.log(' consolidate() , at hour ', date.getHours(), ', merging : \n anticipate (', antic, '),\n program (', program, ') \n and usermanual (', user, ')\n choose to applyed intermedite (', apply, ') ,\n update the relays state into: ', res);
  ...
  function applyIntermed(proposal,smallgapTemp) { // antMap : is the anticInterm2VirtMap in model.js.   proposal is current proposed relays to update if intermediate is set 
                                                ...
                                                // SUMMARY:
                                                // QUINDI ho 2 proposte proposal e nel una proposta per il caso in cui il itermediate var gaspdcPref attiva il anticipate mode
                                                //  comunque la proposta iesima va applicata solo se e' settato true nel corrispondete item di antMap.gaspdcPref
                                                // return il proposal updatato
      ...
      for (ant in antMap) {// ant is an itermediate var registered in antMap , example: ant=gaspdcPref
        ...
        if ((ind = state.app.plantconfig.relaisEv.indexOf(ant)) >= 0)// ind : index of ant
          if (proposal[ind])// the proposal says: if intermediate of index ind (name=gaspdcPref) is proposed true >> then apply its consequences described in antMap[ant]
          {
            ...
            antMap[ant].forEach((val, ind) => {// force any (not  null) interm var PROPOSAL: [true,true,true,null,null,true,null] if intermediate algo activated it : smallgapTemp[ind].smallgap=true
              if (val == true)// if is true  apply the anticipate proposal 
              if (smallgapTemp[ind]!=null) proposal[ind] = smallgapTemp[ind]; 
            });
            return true
          }
      }
...
function  valCorrection(value){// change 0 <> 1 user only in raspberry because the gpio had inverted on/off in connecting the port to rele
  if(PRTLEV>7)console.log('valCorrection() value: ',value);
