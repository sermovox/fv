...
let PRTLEV=9;// a context set by fv3. to pass to hawsclient !
...
const hass_=require("../../homeassistant_new/node_modules/homeassistant-ws/build/index.js");// install and modify node_modules/homeassistant-ws/build/index.js in a specific directory homeassistant-new
...
async function getNewCon(cfg_){// called by : async function kepAlive(reset=false)
...
if(connCfg&&connCfg.token&&connCfg.host&&connCfg.port){
  client = await hass_.createClient(
    /*connCfg={  // ..... LLHH resolve with clientobj=clientObject(client)
    token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJiNmYxMDk3NDYxODI0YmZhYjkwNTc2NjQ1ZDVmODU4MyIsImlhdCI6MTY5NDQ0Mjg2MiwiZXhwIjoyMDA5ODAyODYyfQ.ppeuf-Ma1vLVQCT0Qrt07C5TXGHsHasX3ElOl1NCX3A', 
    host: '192.168.1.212',
    port: 8123,
...
async function main() {// call in a connected client to init/restart the data transmission
  ...
  if(PRTLEV>8)console.log(' hawsclient: main() inital states are: ',inState);
...
async function kepAlive(reset=false,cfg){// if client==null: start tracking a new connecting client, use reset x ....
                                      // returns false if client is not (re)started
    if(reset)client=ws=null;
    if(client!=null)return true;
    await getNewCon(cfg);// fills client and ws with a new connection 
    if(client){
...
function docmd(topic,reason,...reads){// cmd,...arguments call ex: docmd(topic,reason,'ser','switch','turn_on','switch.rssi') docmd(topic,reason,'ser','python_script','set_state','switch.rssi','on')
...
const controller={ /*
                    ...
                    init:// 
                        function (ctlcb_,PRTLEV_,plantconfig//,ctlComTopic_
                        ){
                          PRTLEV=PRTLEV_;hass_.setctx(PRTLEV);
                         ...
                         if(plantconfig.connCfg){ kepAlive(true,plantconfig.connCfg);
                          // if really connected :
                          return true;
                         }
                         return false;
